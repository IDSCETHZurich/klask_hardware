...
void Player::update_magnet_position(const std::vector<float>& encoder_values) {
    if (!synchronized_state.empty()) {
        magnet_position[0] = synchronized_state[2] + 
            (1.0f / 2.0f) * ((encoder_values[0] - synchronized_state[0]) * DISTANCE_PER_REVOLUTION - 
                             (encoder_values[1] - synchronized_state[1]) * DISTANCE_PER_REVOLUTION) * REAL_TO_CAM_FACTOR_X;
        magnet_position[1] = synchronized_state[3] + 
            (1.0f / 2.0f) * ((encoder_values[0] - synchronized_state[0]) * DISTANCE_PER_REVOLUTION + 
                             (encoder_values[1] - synchronized_state[1]) * DISTANCE_PER_REVOLUTION) * REAL_TO_CAM_FACTOR_Y;
    }
    magnet_velocity[0] = (1.0f / 2.0f) * (encoder_values[2] * DISTANCE_PER_REVOLUTION - 
                                          encoder_values[3] * DISTANCE_PER_REVOLUTION) * REAL_TO_CAM_FACTOR_X;
    magnet_velocity[1] = (1.0f / 2.0f) * (encoder_values[2] * DISTANCE_PER_REVOLUTION + 
                                          encoder_values[3] * DISTANCE_PER_REVOLUTION) * REAL_TO_CAM_FACTOR_Y;
